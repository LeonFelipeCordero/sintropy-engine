...
class ConnectionRouterTest {
    ...
    @Test
    fun `should add new connection with routing`() =
        runTest {
    ...
    @Test
    fun `should add new connection with new routing key to existing channel`() =
        runTest {
    ...
    @Test
    fun `should add new connection to existing routing key to existing channel`() =
        runTest {
            val connectionId1 = UUID.randomUUID().toString()
            val connectionId2 = UUID.randomUUID().toString()
            connectionRouter.add(connectionId1, defaultChannel.name, DEFAULT_ROUTING_KEY)
    ...
    @Test
    fun `should not add same connection twice`() =
        runTest {
            val connectionId = UUID.randomUUID().toString()
            connectionRouter.add(connectionId, defaultChannel.name, DEFAULT_ROUTING_KEY)
    ...
    @Test
    fun `should remove consumer from routing table`() =
        runTest {
            val connectionId = UUID.randomUUID().toString()
            connectionRouter.add(connectionId, defaultChannel.name, DEFAULT_ROUTING_KEY)
    ...
    @Test
    fun `should remove consumer twice from routing table and not fal`() =
        runTest {
            val connectionId = UUID.randomUUID().toString()
            connectionRouter.add(connectionId, defaultChannel.name, DEFAULT_ROUTING_KEY)
    ...
    @Test
    fun `should spread keys when multiple coroutines write at the same time`() =
        runTest {
            ...
            val launchLastLevel: suspend () -> Unit = {
                val pair =
                    when (Random.nextInt(1, 4)) {
                        1 -> {
                            Triple(channel1.channelId!!, channel1.name, channel1.routingKeys[Random.nextInt(0, 3)])
                        }

                        2 -> {
                            Triple(channel2.channelId!!, channel2.name, channel2.routingKeys[Random.nextInt(0, 2)])
                        }

                        3 -> {
                            Triple(channel3.channelId!!, channel3.name, channel3.routingKeys.first())
                        }
