...
class MessageRecoveryTest : IntegrationTestBase() {
    ...
    @Test
    fun `should connect and receive ready to stream response`() = runTest {
    ...
    @Test
    fun `should stream all messages when streamAll is true`() = runTest {
        ...
        val request = RecoveryStreamRequest(
            batchSize = 10,
            delayInMs = 50,
            streamAll = true
        )
    ...
    @Test
    fun `should stream messages within time range when from and to are provided`() = runTest {
        val message1 = publishMessage(channel, producer)
        Thread.sleep(100)
        val message2 = publishMessage(channel, producer)
        Thread.sleep(100)
    ...
    @Test
    fun `should stream messages from a given time without end time`() = runTest {
        val message1 = publishMessage(channel, producer)
        Thread.sleep(50)
        val message2 = publishMessage(channel, producer)
        Thread.sleep(50)
    ...
    @Test
    fun `should return empty when no messages match the time range`() = runTest {
    ...
    @Test
    fun `should return error when channel does not exist`() = runTest {
        val nonExistentUri =
    ...
    @Test
    fun `should return error when routing key does not exist`() = runTest {
    ...
    @Test
    fun `should return error when from is missing and streamAll is false`() = runTest {
        var errorResponse: StreamingErrorResponse? = null
        val latch = CountDownLatch(1)
    ...
    @Test
    fun `should stream messages in batches`() = runTest {
        repeat(15) {
            publishMessage(channel, producer)
        }
    ...
    @Test
    fun `should only stream messages for the specified routing key`() = runTest {
    ...
    @Test
    fun `should close connection properly`() = runTest {
