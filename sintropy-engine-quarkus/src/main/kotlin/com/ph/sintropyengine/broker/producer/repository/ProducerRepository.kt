...
class ProducerRepository(
    ...
    fun findById(id: UUID): Producer? =
        context.selectFrom(Tables.PRODUCERS)
    ...
    fun findByChannel(channelName: String): List<Producer> =
        context.select(Tables.PRODUCERS.asterisk())
            .from(Tables.PRODUCERS)
            .leftJoin(Tables.CHANNELS).on(Tables.PRODUCERS.CHANNEL_ID.eq(Tables.CHANNELS.CHANNEL_ID))
            .where(Tables.CHANNELS.NAME.eq(channelName))
            .fetchInto(Producer::class.java)

    fun findAll(): List<Producer> =
        context.selectFrom(Tables.PRODUCERS).fetchInto(Producer::class.java)

    fun delete(id: UUID) =
        context.deleteFrom(Tables.PRODUCERS)
            .where(Tables.PRODUCERS.PRODUCER_ID.eq(id))
            .execute()
