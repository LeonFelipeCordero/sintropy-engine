defmodule SintropyEngine.FifoQueueTest do
  ...
  describe "polling from fifo queue" do
    test "should queue a message and poll" do
      message = message_fixture()

      channel_id = UUID.dump!(message.channel_id)

      polled_message = Messages.poll_fifo(channel_id, message.routing_key)
    ...
    test "should not return anything if the message is consumed" do
      ...
      Messages.poll_fifo(channel_id, message.routing_key)
      polled_message = Messages.poll_fifo(channel_id, message.routing_key)
    ...
    test "should queue two messages and poll one by one" do
      ...
      polled_message1 = Messages.poll_fifo(channel_id1, message1.routing_key)
      Messages.dequeue(Enum.at(polled_message1, 0))
      polled_message2 = Messages.poll_fifo(channel_id2, message2.routing_key)
    ...
    test "should not poll any message if the previous poll hasn't confirm anything" do
    end
    ...
    test "should queue a message and try to poll five" do
      ...
      polled_message = Messages.poll_fifo(channel_id, message.routing_key, 5)
    ...
    test "should poll from an empty queue and do not fail" do
      ...
      polled_message = Messages.poll_fifo(channel_id, "default.routing.key")
    ...
    test "should poll messages in chronological order one by one" do
      ...
      polled_message1 = Messages.poll_fifo(channel_id, routing_key)
      Messages.dequeue(Enum.at(polled_message1, 0))
      polled_message2 = Messages.poll_fifo(channel_id, routing_key)
      Messages.dequeue(Enum.at(polled_message2, 0))
      polled_message3 = Messages.poll_fifo(channel_id, routing_key)
    ...
    test "should poll messages in chronological order all at once" do
      ...
      polled_messages = Messages.poll_fifo(channel_id, routing_key, 3)
    ...
    test "should not poll messages from other routing key" do
      ...
      polled_message1 = Messages.poll_fifo(channel_id, routing_key1, 2)
      polled_message2 = Messages.poll_fifo(channel_id, routing_key2, 2)
    ...
    test "should not poll messages from other channel" do
      ...
      polled_message1 = Messages.poll_fifo(channel_id1, message1.routing_key, 2)
      polled_message2 = Messages.poll_fifo(channel_id2, message2.routing_key, 2)
    ...
    test "should not pull a message that has been poll more than three times" do
      ...
      polled_messages =
        Messages.poll_fifo(UUID.dump!(message.channel_id), message.routing_key)
    ...
    # TODO there is not time limit to make a message available again
    # test "should not pull a message is lass than 15 minutes pulled" do
    #   message = message_fixture()
    #
    #   channel_id = UUID.dump!(message.channel_id)
    #
    #   first_poll = Messages.poll_fifo(channel_id, message.routing_key)
    #   assert length(first_poll) == 1
    #
    #   second_poll = Messages.poll_fifo(channel_id, message.routing_key)
    ...
    #   })
    #
    #   third_poll = Messages.poll_fifo(channel_id, message.routing_key)
    ...
    test "concurrent processing of messages" do
      testing_data = testing_data_fifo()
      ...
      |> Enum.each(fn {key, messages} ->
        ...
        messages_ids = messages |> Enum.map(& &1.id)
